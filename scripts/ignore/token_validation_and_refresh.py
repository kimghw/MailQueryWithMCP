...
class TokenValidationRefreshTest:
    ...
    async def setup_test_data(self):
        ...
        account = self.db.fetch_one(
            "SELECT * FROM accounts WHERE user_id = ?", (self.test_user_id,)
        )

        if account:
            logger.info(f"기존 계정 발견: {self.test_user_id}")
            logger.info(
                f"계정 상태: {account['status'] if 'status' in account.keys() else 'N/A'}"
            )
            logger.info(
                f"토큰 만료시간: {account['token_expiry'] if 'token_expiry' in account.keys() else 'N/A'}"
            )
            logger.info(
                f"활성 상태: {account['is_active'] if 'is_active' in account.keys() else 'N/A'}"
            )
        else:
    ...
    async def test_scenario_1_valid_token(self):
        ...
        try:
            # 토큰 유효성 검증 및 갱신
            result = await self.token_service.validate_and_refresh_token(
                self.test_user_id
            )
    ...
    async def test_scenario_2_expired_token_with_valid_refresh(self):
        ...
        try:
            ...
            if result["status"] == "refreshed":
                ...
                if updated_account:
                    logger.info(
                        f"새로운 토큰 만료시간: {updated_account['token_expiry']}"
                    )
    ...
    async def test_scenario_5_force_token_refresh(self):
        ...
        try:
            # 강제 토큰 갱신
            refresh_result = await self.token_service.force_token_refresh(
                self.test_user_id
            )
