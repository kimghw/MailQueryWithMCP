...
async def clear_all_data():
    ...
    try:
        ...
        with db.transaction():
            ...
            logger.info(f"'mail_history' 테이블에서 {history_rows}개 행이 삭제되었습니다.")

            logger.info("'processing_logs' 테이블 데이터 삭제 중...")
            log_rows = db.delete("processing_logs", "1=1")
            logger.info(f"'processing_logs' 테이블에서 {log_rows}개 행이 삭제되었습니다.")

            logger.info("'account_audit_logs' 테이블 데이터 삭제 중...")
            audit_log_rows = db.delete("account_audit_logs", "1=1")
            logger.info(f"'account_audit_logs' 테이블에서 {audit_log_rows}개 행이 삭제되었습니다.")
