...
class AllAccountsMailQueryTester:
        ...
        try:
            ...
            for mail in response.messages[:5]:
                ...
                result["messages"].append({
                    "id": mail.id,
                    "subject": mail.subject[:80] + "..." if len(mail.subject) > 80 else mail.subject,
                    "sender": sender,
    ...
    async def test_all_accounts(self, days_back: int = 60, max_mails_per_account: int = 10):
        """모든 계정 메일 조회 테스트"""
        ...
        for i, account in enumerate(accounts, 1):
            ...
            if result['success']:
                ...
                print(f"  ✅ 성공: {result['total_mails']}개 메일 ({result['execution_time_ms']}ms)")
        ...
        for result in all_results:
            ...
            print(f"{result['user_id']:<20} {result['total_mails']:>10} "
        ...
        print(f"\n⏱️  실행 시간 분석:")
        print(f"  - 총 실행 시간: {total_execution_time}ms ({total_execution_time/1000:.2f}초)")
...
async def main():
    ...
    try:
        ...
        await tester.test_all_accounts(
            days_back=60,              # 최근 60일
            max_mails_per_account=10   # 계정당 10개
        )
