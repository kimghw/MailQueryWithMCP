...
class AllAccountsFullProcessTester:
    ...
    def __init__(self):
        ...
        self.duplicate_check_enabled = self.config.get_setting(
            "ENABLE_MAIL_DUPLICATE_CHECK", "true"
        ).lower() == "true"
        
        # 키워드 추출 설정 확인
        self.structured_extraction = self.config.get_setting(
            "ENABLE_STRUCTURED_EXTRACTION", "true"
        ).lower() == "true"
        
        self.batch_extraction = self.config.get_setting(
            "ENABLE_BATCH_KEYWORD_EXTRACTION", "true"
        ).lower() == "true"
        
        logger.info(f"🔍 중복 체크: {'활성화' if self.duplicate_check_enabled else '비활성화'}")
        logger.info(f"🔍 구조화된 추출: {'활성화' if self.structured_extraction else '비활성화'}")
        logger.info(f"🔍 배치 추출: {'활성화' if self.batch_extraction else '비활성화'}")
    ...
    async def check_existing_mails(self, user_id: str) -> Dict[str, int]:
        ...
        try:
            ...
            return {
                ...
                "by_month": {row['month']: row['count'] for row in monthly_stats} if monthly_stats else {}
            }
        except Exception as e:
    ...
    async def process_account(
        ...
        try:
            ...
            logger.info(f"   - 중복 체크: {'ON' if self.duplicate_check_enabled else 'OFF'}")
            logger.info(f"   - 구조화 추출: {'ON' if self.structured_extraction else 'OFF'}")
            ...
            if "structured_data_samples" in process_stats:
                result["structured_data_samples"] = process_stats["structured_data_samples"][:3]  # 최대 3개
    ...
    async def test_all_accounts(
        ...
        print(f"  - 구조화 추출: {'활성화' if self.structured_extraction else '비활성화'}")
        ...
        for i, account in enumerate(accounts, 1):
            ...
            total_stats["total_keywords_extracted"] += result["processing_stats"]["keywords_extracted"]
    ...
    def _print_account_result(self, result: Dict[str, Any]):
        ...
        print(f"     - 키워드 추출: {result['processing_stats']['keywords_extracted']}개")
    ...
    def _print_total_stats(self, total_stats: Dict[str, Any], all_results: List[Dict[str, Any]]):
        """전체 통계 출력"""
        print("\n" + "=" * 80)
        print("📊 전체 통계")
        print("=" * 80)
    ...
    def _print_result_table(self, all_results: List[Dict[str, Any]]):
        ...
        if self.duplicate_check_enabled:
            print(f"{'계정':<15} {'기존':<8} {'조회':<8} {'저장':<8} {'중복':<8} {'필터링':<8} {'이벤트':<8} {'키워드':<8} {'시간(초)':<10}")
            print("-" * 101)
            ...
            print(f"{'계정':<15} {'조회':<8} {'필터링':<8} {'이벤트':<8} {'키워드':<8} {'시간(초)':<10}")
            print("-" * 68)
...
async def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--help":
            ...
            print("  ENABLE_STRUCTURED_EXTRACTION     구조화된 추출 활성화 (기본: true)")
