...
class AllAccountsFullProcessTester:
    ...
    def __init__(self):
        ...
        logger.info(
            f"🔍 중복 체크 설정: {'활성화' if self.duplicate_check_enabled else '비활성화'}"
        )
    ...
    async def process_account(
        ...
        try:
            ...
            logger.info(
                f"🔧 [{user_id}] 메일 처리 시작 (중복 체크: {'ON' if self.duplicate_check_enabled else 'OFF'})..."
            )
    ...
    async def test_all_accounts(
        ...
        for i, account in enumerate(accounts, 1):
            ...
            if self.duplicate_check_enabled:
                print(
                    f"     - 기존 저장 메일: 전체={result['existing_mails']['total']}개"
                )
        ...
        if self.duplicate_check_enabled:
            print(
                f"{'계정':<15} {'기존':<8} {'조회':<8} {'저장':<8} {'중복':<8} {'필터링':<8} {'이벤트':<8} {'시간(초)':<10}"
            )
            print("-" * 93)
            ...
            print(
                f"{'계정':<15} {'조회':<8} {'필터링':<8} {'이벤트':<8} {'시간(초)':<10}"
            )
            print("-" * 60)
...
async def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--help":
            print(
                "사용법: python mail_query_process.py [days] [max_mails] [--test-duplicate user_id]"
            )
