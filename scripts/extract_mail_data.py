...
def extract_mail_data_to_txt():
    ...
    try:
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            for i, (mail_id, subject, sender, keywords, received_time) in enumerate(rows, 1):
        ...
        with open(stats_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"제목이 있는 메일: {sum(1 for _, subject, _, _, _ in rows if subject)}\n")
            f.write(f"발신자가 있는 메일: {sum(1 for _, _, sender, _, _ in rows if sender)}\n")
            f.write(f"키워드가 있는 메일: {sum(1 for _, _, _, keywords, _ in rows if keywords)}\n")
            f.write(f"고유 발신자 수: {len(set(sender for _, _, sender, _, _ in rows if sender))}\n")
            ...
            # 날짜 범위
            dates = [received_time for _, _, _, _, received_time in rows if received_time]
            if dates:
