...
class DashboardEventService:
    ...
    def __init__(self):
        ...
        # 설정값
        self.dashboard_events_enabled = (
            self.config.get_setting("ENABLE_DASHBOARD_EVENTS", "true").lower() == "true"
        )
    ...
    async def publish_extraction_result(
        ...
        try:
            ...
            # 이벤트 데이터 생성
            event_data = self._create_single_event(
                mail_id, structured_result, default_metadata
            )
            ...
            self.logger.error(
                f"대시보드 이벤트 발행 중 예외: mail_id={mail_id}, error={str(e)}",
                exc_info=True,
            )
            return False
    ...
    def _is_dashboard_worthy(self, structured_result: Dict[str, Any]) -> bool:
        ...
        # 조건 1: 아젠다 번호가 있어야 함
        has_agenda = bool(structured_result.get("agenda_no"))

        # 조건 2: 발신자 조직이 IACS 멤버여야 함
        ...
        has_iacs_sender = sender_org in iacs_members

        # 핵심 변경: 둘 다 만족해야 함 (AND 조건)
        result = has_agenda and has_iacs_sender

        # 디버깅을 위한 로그 (조건별 상세 정보)
        if not result:
            self.logger.debug(
                f"대시보드 이벤트 조건 불충족 - "
                f"agenda_no: {structured_result.get('agenda_no', 'None')}, "
                f"sender_org: {sender_org}, "
                f"has_agenda: {has_agenda}, "
                f"has_iacs_sender: {has_iacs_sender}"
            )
        else:
            self.logger.debug(
                f"대시보드 이벤트 조건 충족 - "
                f"agenda_no: {structured_result.get('agenda_no')}, "
                f"sender_org: {sender_org}"
            )
