    ...
    async def _handle_mail_query(self, arguments: Dict[str, Any]) -> str:
        ...
        try:
            ...
            for i, mail in enumerate(response.messages, 1):
                ...
                result_text += (
                    f"   수신일: {mail.received_date_time.strftime('%Y-%m-%d %H:%M')}\n"
                )
...
"""
        ...
        with open(csv_file, "w", newline="", encoding="utf-8-sig") as f:
            ...
            for idx, email in enumerate(emails, 1):
                ...
                row = {
                    ...
                    "첨부파일": (
                        "있음" if email.get("has_attachments", False) else "없음"
                    ),
    ...
    async def _handle_streaming_request(self, request: Request):
        ...
        if method == "initialize":
            ...
            logger.info(
                f"📤 Sending initialize response: {json.dumps(response, indent=2)}"
            )
            ...
            logger.info(
                f"📤 Returning {len(tools_data)} tools: {[t['name'] for t in tools_data]}"
            )
            ...
            logger.info(
                f"  • Arguments: {json.dumps(tool_args, indent=2, ensure_ascii=False)}"
            )
    ...
    def run(self):
        ...
        logger.info(
            f"🚀 Starting HTTP Streaming Mail Attachment Server on http://{self.host}:{self.port}"
        )
