...
class StatisticsService:
    ...
    async def record_statistics(
        ...
        statistics = {
            ...
            'success_rate': self._calculate_success_rate(
                ...
                new_count  # 신규 메일 대비 성공률
            ),
            'duplication_rate': self._calculate_duplication_rate(
                duplicate_count,
                filtered_count  # 필터링된 메일 대비 중복률
            ),
            'keywords': keywords,
            'unique_keywords': list(set(keywords)),
            'keyword_count': len(keywords),
            'unique_keyword_count': len(set(keywords)),
            'duplicate_check_enabled': saved_results.get('duplicate_check_enabled', True),
            'processing_efficiency': self._calculate_efficiency(
                new_count, duplicate_count, len(processed_mails) if processed_mails else 0
            ),
    ...
    def _calculate_efficiency(self, new_count: int, duplicate_count: int, processed_count: int) -> float:
    ...
    async def get_processing_efficiency_report(self, account_id: str, days: int = 7) -> Dict:
        """처리 효율성 리포트 생성"""
        try:
            ...
            for row in results:
                duplicate_rate = 0
                if row['mail_count'] > 0:
                    duplicate_rate = ((row['mail_count'] - row['unique_content_count']) / row['mail_count']) * 100
            ...
            return {
                ...
                'average_duplicate_rate': round(
                    sum(d['duplicate_rate'] for d in efficiency_data) / len(efficiency_data) 
                    if efficiency_data else 0, 2
                )
            }
