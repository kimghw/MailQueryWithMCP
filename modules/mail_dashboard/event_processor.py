...
class EmailDashboardEventProcessor:
    ...
    def process_email_event(self, event_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 3. agenda_code가 없으면 pending 처리
            if not event.event_info.agenda_code:
                self._save_to_pending(event, "no_agenda_code", "아젠다 코드 없음")
                return {
                    "success": True,
                    "action": "saved_to_pending",
                    "reason": "no_agenda_code",
                }
    ...
    def _process_member_response(self, event: EmailReceivedEvent) -> Dict[str, Any]:
        ...
        try:
            ...
            if not response_org or response_org not in ORGANIZATIONS:
                ...
                self._save_to_pending(
                    event, "invalid_organization", f"유효하지 않은 조직: {response_org}"
                )
                return {
                    "success": True,
                    "action": "saved_to_pending",
