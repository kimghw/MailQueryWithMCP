#!/usr/bin/env python3
"""
Query matcher with synonym support and multiple similar results
"""
import json
import sqlite3
import numpy as np
...
class QueryMatcherV3:
    ...
    def find_best_matches(self, query: str, top_k: int = 3) -> Tuple[List[Dict], List[Dict]]:
        """
        Find best matching templates with synonym support
        Returns: (primary_matches, similar_matches)
        - primary_matches: Top match
        - similar_matches: Additional matches within 3% of top score
        """
        ...
        for template_id, template_data in self._template_cache.items():
            ...
            matched_question = template_data['natural_questions'][matched_idx]
        ...
        # Find all matches within tolerance (3%) of top score
        primary_matches = [top_match]
        similar_matches = []
        
        for match in filtered[1:top_k]:
    ...
    def _load_templates(self):
        ...
        for row in cursor.fetchall():
            ...
            # Try to get description from SQL query (system prompt)
            description = ""
            if sql_query:
                # Simple extraction - can be improved
                description = f"Query for {category}"
