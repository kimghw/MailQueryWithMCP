...
from .account_schema import (
    EnrollmentFileData, AccountCreate, AccountUpdate, AccountSyncResult,
    AccountStatus, AuthType, OAuthConfig
)
...
class AccountSyncService:
    ...
    def account_sync_all_enrollments(self) -> AccountSyncResult:
        ...
        try:
            ...
            enrollment_files = self.file_helper.account_list_enrollment_files(enrollment_dir)
            ...
            for file_path in enrollment_files:
                try:
                    ...
                    if sync_file_result['success']:
                        ...
                        logger.info(f"enrollment 동기화 성공: {file_path} -> {sync_file_result['action']}")
                    else:
                        result.errors.append(f"{file_path}: {sync_file_result['error']}")
                        logger.error(f"enrollment 동기화 실패: {file_path} -> {sync_file_result['error']}")
            ...
            deactivated_count = self._account_deactivate_orphaned_accounts(processed_user_ids)
            result.deactivated_accounts = deactivated_count
            
            logger.info(f"enrollment 동기화 완료: 생성={result.created_accounts}, "
    ...
    def _account_sync_single_enrollment(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            enrollment_data = self._account_parse_enrollment_file(file_data, file_path, file_hash)
            ...
            if existing_account:
                # 기존 계정 업데이트
                action = self._account_update_existing_account(existing_account, enrollment_data)
                return {
                    'success': True,
                    'action': action,
                    'user_id': user_id,
    ...
    def _account_parse_enrollment_file(self, file_data: Dict[str, Any], 
                                     file_path: str, file_hash: str) -> EnrollmentFileData:
        """
    ...
    def _account_create_new_account(self, enrollment_data: EnrollmentFileData, user_id: str) -> int:
        """
        새로운 계정 생성
    ...
    def _account_update_existing_account(self, existing_account, enrollment_data: EnrollmentFileData) -> str:
        """
        기존 계정 업데이트
        ...
        if existing_account.enrollment_file_hash == enrollment_data.file_hash:
            logger.debug(f"파일 해시 동일 - 업데이트 불필요: {existing_account.user_id}")
            return 'no_change'
    ...
    def account_validate_enrollment_file(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                # 추가 비즈니스 로직 검증
                if enrollment_data.oauth_config.client_secret == "YOUR_CLIENT_SECRET_HERE":
                    result['warnings'].append('클라이언트 시크릿이 placeholder 값입니다')
