...
from .account_schema import (
    ...
    AccountAuditLog, AccountStatus, AuthType
)
...
class AccountRepository:
    ...
    def account_create_from_enrollment(self, account_data: AccountCreate) -> int:
        ...
        try:
            ...
            with self.db.transaction() as conn:
                ...
                logger.info(f"계정 생성 완료: ID={account_id}, user_id={account_data.user_id}")
                return account_id
            ...
            with self.db.transaction() as conn:
                ...
                if cursor.rowcount > 0:
                    ...
                    audit_data = self.audit_helper.account_create_audit_message(
                        ...
                        existing_account.dict(), update_data.dict(exclude_unset=True)
                    )
                    self._account_create_audit_log(cursor, audit_data)
    ...
    def _account_row_to_response(self, row: sqlite3.Row) -> AccountResponse:
        ...
        permissions = json.loads(row['delegated_permissions']) if row['delegated_permissions'] else []
    ...
    def _account_check_token_validity(self, row: sqlite3.Row) -> bool:
        ...
        try:
            expiry_time = datetime.fromisoformat(row['token_expiry'].replace('Z', '+00:00'))
