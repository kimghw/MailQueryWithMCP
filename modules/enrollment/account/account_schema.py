...
class AccountCreate(BaseModel):
    ...
    oauth_client_secret: str = Field(
        ..., description="OAuth 클라이언트 시크릿 (암호화됨)"
    )
    ...
    delegated_permissions: List[str] = Field(
        default_factory=list, description="위임된 권한"
    )
...
class AccountAuditLog(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="변경 시간"
    )
...
class AccountSyncResult(BaseModel):
    ...
    sync_time: datetime = Field(
        default_factory=datetime.utcnow, description="동기화 시간"
    )
