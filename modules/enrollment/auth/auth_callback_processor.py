...
AuthWebServer에서 분리된 순수 비즈니스 로직입니다.
웹서버와 독립적으로 OAuth 콜백을 처리하며, 재사용 가능한 구조를 제공합니다.
"""
...
class AuthCallbackProcessor:
    ...
    def process_callback(self, query_params: Dict[str, str]) -> str:
        """
        OAuth 콜백을 처리하고 HTML 응답을 반환합니다.

        Args:
            query_params: OAuth 콜백 쿼리 파라미터
                - code: 인증 코드
                - state: CSRF 토큰
                - session_state: 세션 상태 (선택)
                - error: 에러 코드 (선택)
                - error_description: 에러 설명 (선택)

        Returns:
            HTML 응답 문자열 (성공/실패 페이지)
        """
        try:
            # 콜백 데이터 생성
...
# 전역 인스턴스 (선택사항)
_callback_processor = None


def get_auth_callback_processor() -> AuthCallbackProcessor:
    """
    콜백 프로세서 싱글톤 인스턴스를 반환합니다.

    Returns:
        AuthCallbackProcessor 인스턴스
    """
    global _callback_processor
    if _callback_processor is None:
        _callback_processor = AuthCallbackProcessor()
