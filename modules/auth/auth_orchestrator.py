...
from .auth_schema import (
    AuthSession, AuthState, AuthStartRequest, AuthStartResponse,
    AuthStatusResponse, AuthBulkRequest, AuthBulkResponse, AuthBulkStatus,
    AuthCleanupRequest, AuthCleanupResponse
)
from .auth_web_server import get_auth_web_server_manager
from ._auth_helpers import (
    auth_generate_session_id, auth_generate_state_token,
...
class AuthOrchestrator:
    ...
    async def auth_orchestrator_start_authentication(
        ...
        try:
            ...
            auth_log_session_activity(
                ...
                {"user_id": user_id, "oauth_config_type": "account" if oauth_config else "global"}
            )
    ...
    async def auth_orchestrator_bulk_authentication(
        ...
        for user_id in request.user_ids:
            try:
                ...
                if auth_status.get("requires_reauth", True):
                    ...
                    user_statuses.append(AuthBulkStatus(
        ...
        # 통계 계산
        pending_count = sum(1 for status in user_statuses if status.status == AuthState.PENDING)
        completed_count = sum(1 for status in user_statuses if status.status == AuthState.COMPLETED)
        failed_count = sum(1 for status in user_statuses if status.status == AuthState.FAILED)
        
        logger.info(f"일괄 인증 시작: 총 {len(request.user_ids)}명, "
    ...
    async def auth_orchestrator_cleanup_sessions(
        ...
        logger.info(f"세션 정리 완료: {cleaned_sessions}개 정리, {active_sessions}개 활성")
    ...
    async def auth_orchestrator_get_all_accounts_status(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for account in accounts:
                ...
                # 현재 진행 중인 세션 확인
                pending_session = self._find_pending_session_by_user(account_dict["user_id"])
