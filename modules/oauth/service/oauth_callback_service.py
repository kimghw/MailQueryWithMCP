...
class AuthCallbackService:
    ...
    async def process_callback(
        self, query_params: Dict[str, str], session_store: Dict[str, AuthSession]
    ) -> str:
        """
        OAuth 콜백을 처리합니다.
    ...
    async def _handle_callback_success(
        self, callback_data: AuthCallback, session_store: Dict[str, AuthSession]
    ) -> str:
        """
        ...
        try:
            ...
            token_info = await self.oauth_service.exchange_code_for_tokens(
                callback_data.code, oauth_config
            )
