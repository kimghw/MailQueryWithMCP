...
class AuthAccountService:
    ...
    async def get_oauth_config(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if account_dict.get("oauth_client_secret"):
                try:
                    decrypted_secret = (
                        self.crypto_helper.account_decrypt_sensitive_data(
                            account_dict["oauth_client_secret"]
                        )
                    )
                    account_dict["oauth_client_secret"] = decrypted_secret
                    self.logger.debug(
                        f"클라이언트 시크릿 복호화 완료: user_id={user_id}"
                    )
            ...
            self.logger.error(
                f"OAuth 설정 조회 실패: user_id={user_id}, error={str(e)}"
            )
            return None
    ...
    async def get_all_accounts(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for account in accounts:
                ...
                if account_dict.get("token_expiry") and isinstance(
                    account_dict["token_expiry"], str
                ):
                    account_dict["token_expiry"] = datetime.fromisoformat(
                        account_dict["token_expiry"]
                    )

                if account_dict.get("last_sync_time") and isinstance(
                    account_dict["last_sync_time"], str
                ):
                    account_dict["last_sync_time"] = datetime.fromisoformat(
                        account_dict["last_sync_time"]
                    )
    ...
    async def update_account_tokens(
        self, user_id: str, token_info: Dict[str, Any]
    ) -> int:
        """
        계정의 토큰 정보를 업데이트합니다.
        ...
        try:
            ...
            account_id = await self.token_service.store_tokens(
                user_id=user_id, token_info=token_info
            )

            self.logger.info(
                f"토큰 저장 완료: user_id={user_id}, account_id={account_id}"
            )
            return account_id
