#!/usr/bin/env python3
"""
Auth 모듈 완전한 인증 플로우 테스트

포트 5000번을 사용하여 OAuth 콜백을 처리하고,
토큰 교환 및 저장까지 전체 플로우를 테스트합니다.
"""
...
async def test_complete_auth_flow():
    """완전한 인증 플로우 테스트"""
    print("=" * 60)
    print("🚀 Auth 모듈 완전한 인증 플로우 테스트 시작")
    print("=" * 60)
    ...
    try:
        ...
        if existing_account:
            print(f"   기존 계정 발견:")
            print(f"   - 상태: {existing_account['status']}")
            print(f"   - access_token: {'있음' if existing_account['access_token'] else '없음'}")
            print(f"   - refresh_token: {'있음' if existing_account['refresh_token'] else '없음'}")
        else:
            print("   기존 계정 없음")
        ...
        print("\n" + "=" * 60)
        print("🎉 Auth 모듈 완전한 인증 플로우 테스트 성공!")
        print("=" * 60)
        return True
