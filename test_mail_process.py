...
# 실제 모듈 import
from modules.mail_process.utilities.iacs_code_parser import IACSCodeParser, ParsedCode
...
def analyze_parsing_results(
    ...
    for subject in test_subjects:
        ...
        if parsed:
            ...
            # 새로운 속성들은 있는 경우에만 추가
            if hasattr(parsed, "is_special"):
                detail["is_special"] = parsed.is_special
            if hasattr(parsed, "parsing_method"):
                detail["parsing_method"] = parsed.parsing_method
...
def print_results(results: Dict[str, Any]):
    ...
    # 몇 가지 파싱 성공 예시
    print(f"\n[파싱 성공 예시] (처음 10개)")
    for i, detail in enumerate(results["parsed_details"][:10]):
        print(f"\n{i+1}. 제목: {detail['subject']}")
        print(f"   → 코드: {detail['full_code']}")
        print(f"   → 타입: {detail['type']}, 패널: {detail['panel']}")
        if detail["organization"]:
            print(
                f"   → 기관: {detail['organization']}, 응답버전: {detail.get('response_version', 'N/A')}"
            )
        if "parsing_method" in detail:
            print(f"   → 파싱방법: {detail['parsing_method']}")


def test_specific_patterns(parser: IACSCodeParser):
    """특정 패턴 테스트"""
    print("\n" + "=" * 80)
    print("특정 패턴 테스트")
    print("=" * 80)
    ...
    test_cases = [
        # 패턴별 테스트
        ("PL25016aIRa", "붙어있는 형식"),
        ("PL25015_KRa", "언더스코어 형식"),
        ("PL25016a_IRa", "소문자+언더스코어"),
        ("PL25016_aIRa", "언더스코어+소문자"),
        ("pl25016aira", "대소문자 구분 없음"),
        ("PS25003pPLa", "PS 패널"),
        ("JWG-SDT25001a", "JWG 기본"),
        ("JWG-CS25001b_PRa", "JWG 응답"),
        ("Multilateral", "특수 케이스"),
        ("테스트 메일", "예외 메일"),
        ("RE: PL25016_IRa", "회신 접두사"),
        ("IACS SDTP Fw: PL25015_ILa", "복합 접두사"),
    ]

    for test_subject, description in test_cases:
        parsed = parser.parse_line(test_subject)
        if parsed:
            print(f"\n✓ {description}: {test_subject}")
            print(f"  → {parsed.full_code}", end="")
            if hasattr(parsed, "parsing_method") and parsed.parsing_method:
                print(f" (방법: {parsed.parsing_method})", end="")
            print()
        else:
            print(f"\n✗ {description}: {test_subject}")
            print(f"  → 파싱 실패 (예외 메일)")
...
def test_extract_all_patterns(parser: IACSCodeParser):
    """extract_all_patterns 메서드 테스트"""
    print("\n" + "=" * 80)
    print("extract_all_patterns 메서드 테스트")
    print("=" * 80)

    test_cases = [
        {
            "subject": "RE: PL25016_IRa: Test email",
            "body": "This email references PL25015_KRa and JWG-SDT25001a in the content.",
        },
        {
            "subject": "Urgent: PL25007bTLa IMO FAL Meeting",
            "body": "Please review PL25008cILc urgently. Also check PS25003pPLa.",
        },
    ]
    ...
    for i, test_case in enumerate(test_cases):
        ...
        print(f"본문: {test_case['body'][:50]}...")

        result = parser.extract_all_patterns(test_case["subject"], test_case["body"])

        if "extracted_info" in result:
            print(f"추출된 코드: {result['extracted_info'].get('full_code', 'N/A')}")
        if "urgency" in result:
            print(f"긴급도: {result['urgency']}")
        if "is_reply" in result:
            print(f"회신 여부: {result['is_reply']}")
        if "additional_agenda_references" in result:
            print(f"추가 참조: {result['additional_agenda_references']}")


def save_results_to_json(
    results: Dict[str, Any], filename: str = "iacs_parser_test_results.json"
):
    """결과를 JSON 파일로 저장"""
    ...
    results_copy = {
        ...
        "parsed_details": results["parsed_details"][:20],  # 처음 20개만 저장
    }
...
def main():
    """메인 함수"""
    print("IACS 코드 파서 모듈 테스트 시작")
    print(
        f"모듈 경로: /home/kimghw/IACSGRAPH/modules/mail_process/utilities/iacs_code_parser.py"
    )
    ...
    try:
        ...
        # 특정 패턴 테스트
        test_specific_patterns(parser)

        # extract_all_patterns 테스트
        test_extract_all_patterns(parser)
        ...
        # 대소문자 구분 없이 파싱된 케이스 (parsing_method가 있는 경우만)
        if results["by_parsing_method"]:
            case_insensitive_count = sum(
                1
                for method, count in results["by_parsing_method"].items()
                if "case_insensitive" in method
                for _ in range(count)
            )
            if case_insensitive_count > 0:
                print(f"대소문자 구분 없이 파싱된 케이스: {case_insensitive_count}개")
